  ...
  try{
    ...
    const{rows}=await pool.query(
      `INSERT INTO children (id,user_id,name,age,avatar_choice,language)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING id,name,age,avatar_choice,language,created_at`,
      [randomUUID(),req.user.id,i.name,i.age,i.avatarChoice,i.language]
    );
...
childrenRouter.patch('/:id',requireParentGate,async(req,res,next)=>{
  try{
    ...
    const{rows}=await pool.query(
      `UPDATE children SET name=$1,age=$2,avatar_choice=$3,language=$4,updated_at=NOW()
       WHERE id=$5 AND user_id=$6 RETURNING id,name,age,avatar_choice,language,created_at`,
      [i.name??c.name,i.age??c.age,i.avatarChoice??c.avatar_choice,i.language??c.language,req.params.id,req.user.id]
    );
