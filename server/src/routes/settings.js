...
settingsRouter.patch('/',async(req,res,next)=>{
  try{
    ...
    const{rows}=await pool.query(
      `UPDATE parent_settings SET daily_message_limit=$1,media_enabled=$2,memory_enabled=$3,updated_at=NOW()
       WHERE user_id=$4 RETURNING *`,
      [i.dailyMessageLimit??c.daily_message_limit,i.mediaEnabled??c.media_enabled,i.memoryEnabled??c.memory_enabled,req.user.id]
    );
